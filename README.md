...
```json
{"airport": "JFK", "partner": "Delta TechOps", "contents": [{"aircraft_type": "A330-300", "part_number": "OXY-GEN-A330-15", "qty": 1}]}
```
- 정식 Pooling 계약은 없으나 파트너십을 통해 긴급히 자재를 땡겨올 수 있는 외부 창고 더미 데이터입니다. 3단계 최적화 알고리즘 탐색의 대상이 됩니다.
...
- **최적화 탐색 (타 공항/제휴 창고)**: `HL7702 ICN에서 AOG, OXY-GEN-A330-15 필요` (ICN 로컬 재고는 없지만 제휴 창고(JFK)에서 조달하는 소요 시간을 계산하여 최적안으로 추천)
- **Pooling 커버리지**: `HL8259 SIN에서 AOG, HYD-PUMP-737-11 필요`
- **진에어 화물 인가 불가 경고**: `LJ2201 CDG에서 AOG, HYD-PUMP-737-11 필요` (운영사가 진에어이고, CDG 공항은 진에어 Cargo Auth가 없으므로 7단계에서 경고 발생)
- **AI 인보이스 생성**: (위 시나리오로 7단계 도달 시) 7단계 패널에서 `✨ 인보이스 생성` 버튼을 클릭하여, DB에 저장된 정확한 조업사 주소가 반영된 송장 텍스트 확인
